...
var _depot int
var _nodes int
var _nodules_count int
var _iterations int64
var _percentSize int64
var _percent int
var _permutation int64
var _minDistance int
var _fragmet int64
var _route string
var nodules []int
var now time.Time

func getOptimumRoute(data [][]int, depot int) {
	_data = data
	_depot = depot
	_nodes = len(_data)
	...
	_fragmet = _percentSize
	_permutation = 1
	_minDistance = math.MaxInt64
	now = time.Now()

	// create permutation array
	nodules = make([]int, _nodules_count)
	var j int
	var i int
	for i = 0; i < _nodes; i++ {
		if i != _depot {
	...
	fmt.Println("RESULT")
	fmt.Printf("Optimum route : %d,%s,%d\n", _depot, _route, _depot)
	fmt.Printf("Distance      : %d\n", _minDistance)
	fmt.Printf("Elapse Time   : %v\n", time.Now().Sub(now))
}

func getRoute(start int, end int) {
	if start == (end - 1) {
		// validate distance
